...
public class AllBricks {
        ...
        if (ballIntersects(current_brick)) {
          ...
          if (current_brick.strength == 0) {
            ...
            if (current_brick.hasPowerUp) {
              PowerUp pow = new PowerUp(current_brick.brick_rect.getX() + Brick.width / 2,
                  current_brick.brick_rect.getY() + Brick.height / 2);
  ...
  private boolean ballHitsSidesOf(Brick current_brick) {
    return b.ball_circle.getCenterY() >= current_brick.brick_rect.getY()
        && b.ball_circle.getCenterX() <= current_brick.brick_rect.getY() + Brick.height;
  }

  private boolean ballHitsTopOrBottomOf(Brick current_brick) {
    return b.ball_circle.getCenterX() >= current_brick.brick_rect.getX()
        && b.ball_circle.getCenterX() <= current_brick.brick_rect.getX() + Brick.width;
  }

  private boolean ballIntersects(Brick current_brick) {
    return b.ball_circle.intersects(current_brick.brick_rect.getBoundsInLocal());
  }
